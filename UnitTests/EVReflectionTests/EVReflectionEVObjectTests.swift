...
//            XCTAssert(theObject == result2, "Pass")
    ...
    /**
     Archive an object that contains a nullable type with NSKeyedArchiver and read it back with NSKeyedUnarchiver.
     Both objects should be equal. We are using the workaround in TestObject3 to solve the setvalue for key issue in Swift 1.2
    ...
    func testNSCodingWithNullableType() {
        ...
        NSLog("unarchived result object = \(result?.debugDescription ?? "")")
